...
//            ->get();
    ...
    // ajax
    public function ajax_warehouse_quantity(Request $request)
    {
        try {
            if ($request->ajax()) {
                $sanpham_id = $request->input('sanpham_id');
                $mau_id = $request->input('color_id');
                $size_id = $request->input('size_id');
                // dd($request->input());
                $product_ct = DB::table('sanpham_ct')
                    ->where([
                    ...
                    ->select('sanpham_ct.*')
                    ->first();
                // xuất số lượng kho của màu và size 
                $result = [
                    'message' => 'Something bad happened.',
                    'warehouse' => $product_ct ? $product_ct->sanpham_ct_sl : 0
                ];
                //kiểm tra tồn tại size của màu đó
                if ($request->input('type') == 'checkedDisabled') {
                    $sizes = DB::table('sanpham_ct')->select('sanpham_size.size_id', 'sanpham_size.size_ten')->distinct()
                        ->join('sanpham_size', 'sanpham_size.size_id', '=', 'sanpham_ct.size_id')
                        ->where('sanpham_id', $sanpham_id)->get();
                    $returnHTML = view('pages.sanpham.ajax_file.size')
                        ->with(['sizes' => $sizes, 'sanpham_id' => $sanpham_id, 'mau_id' => $mau_id, 'size_id' => $size_id])->render();
                    $result = array_merge($result, ['html' => $returnHTML]);
                }
